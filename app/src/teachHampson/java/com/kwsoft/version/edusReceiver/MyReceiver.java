package com.kwsoft.version.edusReceiver;
...
//        	i.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TOP );
	...
	private void processCustomMessage(Context context, Bundle bundle) {
		if (StuMainActivity.isForeground) {
			String message = bundle.getString(JPushInterface.EXTRA_MESSAGE);
			String extras = bundle.getString(JPushInterface.EXTRA_EXTRA);
			Intent msgIntent = new Intent(StuMainActivity.MESSAGE_RECEIVED_ACTION);
			msgIntent.putExtra(StuMainActivity.KEY_MESSAGE, message);
			if (!ExampleUtil.isEmpty(extras)) {
				try {
					JSONObject extraJson = new JSONObject(extras);
					if (null != extraJson && extraJson.length() > 0) {
						msgIntent.putExtra(StuMainActivity.KEY_EXTRAS, extras);
					}
				} catch (JSONException e) {
